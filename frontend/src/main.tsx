...
function App(){
 ...
 {role==='employee'&&cardId?<SharedView id={cardId} request={api} logout={logout}/>:role==='hr'?<HrDashboard request={api} logout={logout}/>:!profile?<main className="login"><p className="eyebrow">ТВОЯ СЛЕДУЮЩАЯ ГЛАВА</p><h1>Расти в профессии.<br/>Развивай своего героя.</h1><p>Осмысленные шаги к следующему грейду.</p><form onSubmit={e=>{e.preventDefault();if(token===input)refresh();else setToken(input);}}><label>Токен доступа (сотрудник или HR)<input type="password" required value={input} onChange={e=>setInput(e.target.value)} /></label><button disabled={busy}>Войти →</button></form></main>:<main>
 ...
 <Sharing request={api} style={style} revision={profile.revision}/><div className="panel history"><h3>История участия</h3>{profile.history.map((h:any,i:number)=><p key={i}>{h.date} · {h.event_id} · {h.status}</p>)}</div></section></div>
