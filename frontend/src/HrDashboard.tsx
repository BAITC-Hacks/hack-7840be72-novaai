...
export default function HrDashboard({ request, logout, role }: any) {
 ...
 {summary?.as_of_date&&<p className="hint">{tr("Дата среза данных:")} {summary.as_of_date}</p>}
 ...
 <section className="panel"><h2>{tr("Участие в активностях")}</h2>{tr(Object.entries(summary.participation).map(([key, value]: any) => <p key={key}>{tr(({ completed: 'Завершено', skipped: 'Пропущено', declined: 'Отклонено', in_progress: 'В процессе', dropped: 'Прекращено', no_show: 'Не явился', overdue: 'Срок прошёл' } as any)[key])}{tr(": ")}<b>{tr(value)}</b></p>))}<p className="hint">{tr("Пропуск сам по себе не свидетельствует о риске увольнения.")}</p></section></div></>)}
 ...
 {tr(preview && <div className="import-preview"><h3>{tr("Готово к загрузке")}</h3>{preview.as_of_date&&<p>{tr("Дата среза данных:")} {preview.as_of_date} · {tr("Навыки учитывают завершения после последней оценки.")}</p>}<p>{tr("Профилей: ")}{tr(preview.counts.employees)}{tr(" · Событий: ")}{tr(preview.counts.events)}{tr(" · Навыков: ")}{tr(preview.counts.skills)}{tr(" · Записей истории: ")}{tr(preview.counts.history)}</p>{tr(preview.warnings.map((w: string) => <p key={w} className="warning">{tr(w)}</p>))}<p><b>{tr("Импорт полностью заменит датасет, прогресс, демо-баланс Coins и историю обменов.")}</b>{tr(" Повреждённые файлы не изменяют данные.")}</p><button disabled={busy} onClick={apply}>{tr("Заменить датасет")}</button></div>)}
